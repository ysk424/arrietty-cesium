...
bool ARowGeography::Initialize(const FString& ScenePath) {
    ...
    if(!FFileHelper::LoadFileToString(raw,*ScenePath) || !FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(raw),data) || !data.IsValid()) {
        Fail(TEXT("scene_missing"),TEXT("Place data missing / launch with run.ps1")); return false;
    }
    ...
    FString configPath=FPaths::ConvertRelativePathToFull(FPaths::ProjectDir()/TEXT("../../cesium.local.json"));
    ...
    Terrain=GetWorld()->SpawnActorDeferred<ACesium3DTileset>(ACesium3DTileset::StaticClass(),FTransform::Identity);
    Terrain->SetGeoreference(Georeference.Get()); Terrain->SetIonAssetID(int64(terrainId)); Terrain->SetIonAccessToken(token);
    Terrain->SetMaximumScreenSpaceError(8); Terrain->SetCreatePhysicsMeshes(true); Terrain->SetEnableWaterMask(false);
    ...
    auto imagery=NewObject<UCesiumIonRasterOverlay>(Terrain,TEXT("SatelliteImagery"));
    imagery->IonAssetID=int64(imageryId); imagery->IonAccessToken=token;
    Terrain->AddInstanceComponent(imagery); imagery->RegisterComponent(); imagery->Activate(true);
