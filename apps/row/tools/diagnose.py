...
RUN = WORKSPACE / 'logs/row' / ('diagnostic-' + time.strftime('%Y%m%dT%H%M%S') + '-' + str(time.time_ns() % 1000000000))


def tracker(seconds):
    import openvr
    cfg = json.loads((WORKSPACE / 'config/row.local.json').read_text(encoding='utf-8-sig'))
...
async def bluetooth(seconds):
    ...
    cfgpath = WORKSPACE / 'config/row.local.json'
