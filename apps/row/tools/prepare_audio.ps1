...
$logs=Join-Path $workspace 'logs/row'
New-Item -ItemType Directory -Force -Path $logs | Out-Null
$log=Join-Path $logs ('audio-content-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'.log')
& (Join-Path $EngineRoot 'Engine/Binaries/Win64/UnrealEditor-Cmd.exe') $project -run=pythonscript "-script=$(Join-Path $PSScriptRoot 'build_audio.py')" -unattended -nop4 -nosplash -nosound -nohmd -DisablePlugins=OpenXR -NullRHI "-abslog=$log" *> "$log.console.txt"
